...
int ***arr; // 3D array to store pixel
        ...
        for (j = 0; j < width; j++)
        {
            ...
            for (l = -kernel_size; l <= kernel_size; l += kernel_size)
            {
                for (m = -kernel_size; m <= kernel_size; m += kernel_size)
                {
                    ...
                    {
                        avg_red += arr[x_pixel][y_pixel][0];   // red channel value
                        avg_green += arr[x_pixel][y_pixel][1]; // green channel value
                        avg_blue += arr[x_pixel][y_pixel][2];  // blue channel value
                        num_of_pixels++;                       // Increment the count of processed pixels
                    }
                }
            }
            // Compute the average values for the RGB channels
